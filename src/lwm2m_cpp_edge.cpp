...
Resource Resource::Parse(const pugi::xml_node& resource_node) {
    ...
    std::string multiple_instances = resource_node.child("MultipleInstances").value();
    if (multiple_instances == "Single") {
        resource.multiple_instances = false;
    } else {
        resource.multiple_instances = true;
    }

    std::string mandatory = resource_node.child("Mandatory").value();
...
Object Object::Parse(const pugi::xml_node& object_node) {
    ...
    std::string multiple_instances = object_node.child("MultipleInstances").value();
    if (multiple_instances == "Single") {
    ...
    std::string mandatory = object_node.child("Mandatory").value();
    if (mandatory == "Optional") {
